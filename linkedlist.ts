...
export class LinkedListGroup implements ISortable {
 ...
 // Convenience method that returns a Node at a given index
  ...
  swap(leftPos: number, rightPos: number): void {
    ...
    const leftPosition = this.at(leftPos);
    const rightPosition = this.at(rightPos);

    let tempLeft = leftPosition.data
    leftPosition.data = rightPosition.data
    rightPosition.data = tempLeft
  }
